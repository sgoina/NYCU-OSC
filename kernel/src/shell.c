    ...
    while (1) {
        ...
        // command "settimeout"
            ...
            for (int i = 0; i < MAX_FD; i++) {
                if (fdt[i] == 0 && vfs_open("/home/file.txt", O_CREAT, &fdt[i]) == 0){
                    fd = i;
                    break;
                }
            }
            vfs_write(fdt[fd], "Operating Systems Capstone", 26);
            if (fdt[fd]){
                vfs_close(fdt[fd]);
                fdt[fd] = 0;
            }
            ...
            char buf[64] = {0};
            for (int i = 0; i < MAX_FD; i++) {
                if (fdt[i] == 0 && vfs_open("/home/file.txt", 0, &fdt[i]) == 0){
                    fd = i;
                    break;
                }
            }
            vfs_read(fdt[fd], buf, sizeof(buf) - 1);
            if (fdt[fd]){
                vfs_close(fdt[fd]);
                fdt[fd] = 0;
            }
